...
lines = []
Theta = np.array([[0.8, 0.3, 0.15]])
XtrainLen = 0.7
nFeatures = 0

fileReader = open("housePrices.csv",'r')
lines = fileReader.readlines() 											# fileReader is now on EOF


y = np.array([[i.split(',')[2][:-1]] for i in lines], dtype=float)
X =	np.array([i.split(',')[0:2] for i in lines],dtype=float)
